...
state_city <- read_dta("Brazil_2021.dta") %>% 
  select(state_id = prov1t, 
         city_id = municipio1t) %>% 
...
mun <- geobr::read_municipality(code_muni="all", year=2021) %>%
  st_transform(4326) %>% 
...
bra_lapop_21 <- read_dta('Brazil_2021.dta') %>% 
  ...
  select(id = uniq_id, 
         ...
         fam_fina = q10newt,#家庭收入
         etid,#人种
         news_read = gi0n,#通过公共媒体关注新闻频率
...
rm(list = setdiff(ls(), c("bra_lapop_21","bra_lapop_23")))
