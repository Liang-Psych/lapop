...
state_city <- read_dta("Brazil_2023.dta") %>% 
...
mun <- geobr::read_municipality(code_muni="all", year=2022) %>% 
  st_transform(4326) %>% 
...
bra_lapop_23 <- read_dta('Brazil_2023.dta') %>%  
  ...
  edre == 1 ~ 3,
  edre == 2 ~ 6,
  edre == 3 ~ 9, edre == 4 ~ 12, edre == 5 ~ 14, edre == 6 ~ 16,
  TRUE ~ edre
  )) %>% 
...
rm(list = setdiff(ls(), "bra_lapop_23"))
